...
// desync did this
uint16_t max_fail_attempts_b4562301 = 0; // 0 - means infinite
extern uint8_t freerdp_unknown_errors_b4562301;
// <-- desync did this
...
void help(int32_t ext) {
  ...
  // desync did this
  PRINT_EXTEND(ext, "  -a RETRIES                     maximum number of retries per scanned host (RDP only for now)\n"
                    "  -E default/nextpair/nextip     what to do on unknown RDP error\n"
                    "     default  = Default Hydra behavior\n"
                    "     nextpair = Process the next login (user and password) pair\n"
                    "     nextip   = Bypass to the next IP address in the list\n"
                    );
  // <-- desync did this
...
void hydra_increase_fail_count(int32_t target_no, int32_t head_no) {
  ...
  // desync
  if (max_fail_attempts_b4562301) {
    maxfail = max_fail_attempts_b4562301;
  }
  // <-- desync
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 2)
    help(0);
  while ((i = getopt(argc, argv, "a:E:hIq64Rrde:vVl:fFg:L:p:OP:o:b:M:C:t:T:m:w:W:s:SUux:yc:K")) >= 0) {
    switch (i) {
    // desync did this
    case 'a':
      max_fail_attempts_b4562301 = (uint16_t)atoi(optarg);
      break;
    case 'E':
      if (strcasecmp(optarg, "DEFAULT")) {
        freerdp_unknown_errors_b4562301 = 0;
      } else if (strcasecmp(optarg, "NEXTPAIR")) {
        freerdp_unknown_errors_b4562301 = 1;
      } else if (strcasecmp(optarg, "NEXTIP")) {
        freerdp_unknown_errors_b4562301 = 2;
      } else {
        fprintf(stderr,
                  "[ERROR] unknown mode %s for option -E, only supporting "
                  "\"DEFAULT\" (default Hydra mode), \"NEXTPAIR\" (proceed to the next login pair) and \"NEXTIP\" (bypass to next IP address)\n",
                  optarg);
        exit(-1);
      }
      break;
